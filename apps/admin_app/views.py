...
def update_drg_route(request):
    ...
    csv_file = request.FILES["drg_definition"]
    ...
    for line in lines:
        ...
        try:
            ...
            DRG.objects.create(ms_drg=fields[0], drg_description=fields[1])
        except:
...
def add_hospital_route(request):
    try: 
        csv_file = request.FILES["hosp_drg_input"]
    except:
        messages.error(request,"Please enter a CSV file.")
        messages.error(request,"Hospital not added.")
        return redirect("/admin")

    errors = Hospital.objects.hospital_validator(request.POST, csv_file)
    if len(errors) > 0:
        for key, value in errors.items():
            messages.error(request, value)
        messages.error(request,"Hospital not added.")
    ...
    try:
        newHosp = Hospital.objects.create(name=request.POST["hosp_name_input"],longitude=float(request.POST["hosp_long_input"]),latitude=float(request.POST["hosp_lat_input"]),source=request.POST["hosp_source_input"],state=request.POST["hosp_state_input"])
    except:
    ...
    for line in lines:
        ...
        try:
            ...
            HospitalDRG.objects.create(hospital_id=newHosp, drg_id=currentDRG, avg_allowed_charge=float(fields[1]))
        except:
    ...
    for drg in unused_DRGs:
        HospitalDRG.objects.create(hospital_id=newHosp, drg_id=drg, avg_allowed_charge=0)
